...
def write_logs(successful_connections_list, failures, consecutive_failures, source_serial, sink_serial, start_time):
    ...
    with open('%s/%s/connection_times.csv' % (INNER_DIRECTORY, start_time), 'w+') as csvfile:
    ...
    with open('%s/%s/consecutive_failures.csv' % (INNER_DIRECTORY, start_time), 'w+') as csvfile:
        csv_writer = csv.writer(csvfile, delimiter=',',
                                quotechar='|', quoting=csv.QUOTE_MINIMAL)
